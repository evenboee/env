...
`Separator` is used when combining keys. e.g. prefix is "API" and key is "PORT" and separator is "_" the resulting key is "API_PORT". default is "_". 

`Tag` is used to specify the tag to use when getting key. default is "env". 

`SliceSeparator` is used to specify the separator of array and slice values from env. e.g. sep="," - "1,2,3" => [1, 2, 3]. default is ",". 
...
this is a way to opt out of the prefix building done for structs. env key will still include the SetParams.Prefix. Boolean value with key `omit_struct_prefix` or `osp`
...
before setting value the set function will take the address of the value and check if that implements the StringUnmarshaler interface and will use that if it exists. 
